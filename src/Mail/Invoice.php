...
class Invoice extends Mailable implements ShouldQueue
{
    ...
    public function __construct($params = [])
    {
        ...
        $this->subject = __('eleven59.backpack-shop::invoice.mail.title', ['order_no' => $this->order->fancy_invoice_no]);
        if($this->copy) {
            $this->subject = __('eleven59.backpack-shop::invoice.mail.title_copy', ['order_no' => $this->order->fancy_invoice_no]);
        }
    ...
    public function build()
    {
        return $this->replyTo($this->copy ? $this->order->email : config('mail.from.address'), $this->copy ? $this->order->name : config('mail.from.name'))
            ...
            ->view(config('eleven59.backpack-shop.invoice-mail-view', 'eleven59.backpack-shop::email.invoice'))
            ->attach($this->invoice);
    }
}
